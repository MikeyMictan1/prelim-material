...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            # UNDO ----------------
            if run_num != 0:
                undo_input = input("Do you want to undo move? y/n")
                if undo_input == "y":
                    self.__Score = undo_score
                    self.__SymbolsLeft += 1
                    CurrentCell.ChangeSymbolInCell(undo_cell)
            ...
            # UNDO ----------------
            undo_cell = CurrentCell.GetSymbol()
            print(undo_cell)
                ...
                try:
                    ...
                    for P in self.__AllowedPatterns:
                        ...
                        if P.MatchesPattern(PatternString, CurrentSymbol):
                            ...
                            self.__GetCell(StartRow, StartColumn).make_blocked()
                            self.__GetCell(StartRow, StartColumn + 1).make_blocked()
                            self.__GetCell(StartRow, StartColumn + 2).make_blocked()
                            self.__GetCell(StartRow - 1, StartColumn + 2).make_blocked()
                            self.__GetCell(StartRow - 2, StartColumn + 2).make_blocked()
                            self.__GetCell(StartRow - 2, StartColumn + 1).make_blocked()
                            self.__GetCell(StartRow - 2, StartColumn).make_blocked()
                            self.__GetCell(StartRow - 1, StartColumn).make_blocked()
                            self.__GetCell(StartRow - 1, StartColumn + 1).make_blocked()
                            return 10
                except:
                    pass
        return 0
